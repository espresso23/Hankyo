...
public class ProgressDAO {
    ...
    public void saveProgress(int learnerID, int courseID, int contentID, String status) throws SQLException {
        String sql = "MERGE INTO Progress AS target " +
                    "USING (VALUES (?, ?, ?, ?, GETDATE())) AS source (learnerID, courseID, course_contentID, status, lastUpdated) " +
                    "ON target.learnerID = source.learnerID AND target.courseID = source.courseID AND target.course_contentID = source.course_contentID " +
                    "WHEN MATCHED THEN " +
                    "    UPDATE SET status = source.status, lastUpdated = source.lastUpdated " +
                    "WHEN NOT MATCHED THEN " +
                    "    INSERT (learnerID, courseID, course_contentID, status, lastUpdated) " +
                    "    VALUES (source.learnerID, source.courseID, source.course_contentID, source.status, source.lastUpdated);";
    ...
    public String getContentStatus(int learnerID, int courseID, int contentID) throws SQLException {
        String sql = "SELECT status FROM Progress WHERE learnerID = ? AND courseID = ? AND course_contentID = ?";
    ...
    public List<Progress> getCourseProgress(int learnerID, int courseID) throws SQLException {
        ...
        String sql = "SELECT p.*, cc.title as contentTitle, cc.type as contentType " +
                    "FROM Progress p " +
                    "JOIN Course_Content cc ON p.course_contentID = cc.course_contentID " +
        ...
        try (Connection conn = DBConnect.getInstance().getConnection();
            ...
            while (rs.next()) {
                ...
                progress.setContentID(rs.getInt("course_contentID"));
